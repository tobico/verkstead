...
//!
//! macOS keeps one in a launch agent and Windows in the Run key, and the stages
//! that package for them write those arms in [`super`]'s place. Until then the
//! menu item is drawn greyed rather than left off — a menu that says what
//! Verkstead can do and what it cannot is worth more than a box that ticks and
//! does nothing — and that is a state [`super::Startup`] already draws, for the
//! Linux machine that names no configuration directory to keep an entry in.
//!
