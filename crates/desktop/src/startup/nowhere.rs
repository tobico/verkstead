...
//! on a platform with no arm of its own.
//!
//! Linux keeps one in an XDG autostart entry, macOS in a launch agent and
//! Windows in a value under the Run key, which are the three arms [`super`]
//! has; a platform that is none of them is this. The menu item is drawn greyed
//! rather than left off — a menu that says what Verkstead can do and what it
//! cannot is worth more than a box that ticks and does nothing — and that is a
//! state [`super::Startup`] already draws, for the Linux machine that names no
//! configuration directory to keep an entry in.
//!
