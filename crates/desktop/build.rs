//! What `verkstead-desktop.exe` carries besides its code: the icon Explorer,
//! the taskbar and Alt-Tab draw it with, and the version information its
//! properties sheet shows.
//!
//! **Windows alone.** Neither is a thing an executable can hold on the other
//! two platforms: the AppImage names an icon a desktop installs beside it and
//! the app bundle keeps one in `Contents/Resources`, both of which are the
//! packaging's business rather than the binary's. Windows has no packaging —
//! the exe is the whole of what a human downloads (ADR-0012) — so what it does
//! not carry itself, it has not got.
//!
...
fn main() {
    ...
    // a build log. `crates/desktop/tests/desktop.rs` reads the built exe for
    // the artwork as well, so a resource compiler that ran and wrote nothing
    // is caught too.
    resource
        .compile()
