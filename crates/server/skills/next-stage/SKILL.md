...
- Read the roadmap's `ROADMAP.md` beside the brief for the stages around this
  ...
  `CLAUDE.md` or `AGENTS.md` and its `CONTEXT.md` glossary.
...
the only thing that stops the run**, and it stops it naturally: the run waits on
the ask until they answer, from wherever they are.
