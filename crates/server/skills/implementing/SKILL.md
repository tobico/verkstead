---
name: implementing
description: Build work that has already been settled. Use when a Conversation's direction is to implement inline — primed with a Brief and the handoff document the grilling wrote.
---

Build the work described in the prompt: the Brief it started from, and — where
the grilling wrote one — the handoff document that session left on its way out.
