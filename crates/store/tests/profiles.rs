...
use verkstead_store::{
    ...
    skip_review, start_capture, start_conversation, start_grilling, timeline, update_profile,
};
...
async fn a_profile_a_conversation_has_chosen_is_removed_and_nulled_out_of_it() {
    ...
    assert_eq!(
        conversation.grilling_pairing, None,
...
async fn one_profile_can_be_both_of_a_conversations_choices() {
    ...
    let conversation = load_conversation(&pool, id).await.unwrap().unwrap();
    assert_eq!(
        conversation.grilling_pairing.as_ref().map(|p| p.profile.id),
        Some(only.id)
    );
    assert_eq!(
