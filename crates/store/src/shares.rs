...
//! is worth keeping about it is the link and the moment, which is what the
//! comment on a pull request carries and what the workbench draws beside the
//! Share row.
//!
...
pub struct Share {
    /// The page a reader is sent to, as GitHub gave it — whole, so that nothing
    /// downstream has to build one out of an id and a guess about where gists
    /// live.
    pub url: String,
