...
const EVENT_KEYED: &[&str] = &[
    ...
    "commit_summaries",
    // And the companion rows a steer asked for, the state it came out of, and
    // what its checkouts already held — all of which hang off the row that says
    // what else it settled, so these go before it and it goes with the rest of
    // those.
    "steer_additions",
    "steer_upgrades",
    "steer_sources",
