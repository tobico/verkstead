...
read off the Capture and the Transcript, kept in one place because the wording is
the backend's and will move — the same bargain the idle signature makes. A
session is read against its own backend's sentence and against no other, so a
wording one of them stops on is one another may print in the middle of its work.
The session is ended along with the stop, the agent's own wait for the same reset
