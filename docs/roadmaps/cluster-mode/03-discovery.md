...
Devices you have not typed appear under **Discovered** in the Devices section,
...
peers. Members are left out of the list. Demonstrable end to end: start a second
server, open Remote access on the first, press Add on the row that appeared.
...
- **Tailscale by probing peers**: the peer list from `tailscale status
  ...
  timeout, each time Remote access is opened rather than on a schedule.
...
2. **Browsing and the Discovered list** — a browse held while Remote access is
...
3. **The Tailscale probe** — `Peer` read from the status JSON, online peers
   probed on the peer port when Remote access opens, results merged with mDNS
   by id.
...
- The Devices section from stage 01 still reads through one hook; discovery may
